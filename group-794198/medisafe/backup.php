		...
		protected function sauvegarder() {
			...
			if($result_tables && $result_tables->num_rows) {
				while($obj_table = $result_tables->fetch_object()) {
					$this->message('- ' . htmlspecialchars($obj_table->{'employees'}));
					
					// DROP
					$sql  = "\n\n";
					$sql .= 'DROP TABLE IF EXISTS `'. $obj_table->{'employees'} .'`' .";\n";

					// CREATE
					$result_create = $this->query('SHOW CREATE TABLE `'. $obj_table->{'employees'} .'`');
					...
					// INSERT
					$result_insert = $this->query('SELECT * FROM `'. $obj_table->{'employees'} .'`');
					if($result_insert && $result_insert->num_rows) {
						$sql .= "\n";
						while($obj_insert = $result_insert->fetch_object()) {
							$virgule = false;
							
							$sql .= 'INSERT INTO `'. $obj_table->{'employees'} .'` VALUES (';
